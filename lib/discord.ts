...
export async function getDiscordInviteSummary(): Promise<DiscordInviteSummary> {
  try {
    ...
    const data = (await response.json()) as { approximate_presence_count?: number };
    return { online: typeof data.approximate_presence_count === "number" ? data.approximate_presence_count : null };
  } catch {
    return { online: null };
  }
}
